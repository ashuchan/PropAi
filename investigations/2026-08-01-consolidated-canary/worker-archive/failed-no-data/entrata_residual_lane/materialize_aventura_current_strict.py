#!/usr/bin/env python3
"""Materialize strict current evidence for Aventura Apartments."""
...
LISTING_CAPTURE = OUT / "hb_deep_inspect_5735_conventional_current.json"
CAPTURE = OUT / "hb_deep_inspect_5735_2bed_modal_current.json"
EVIDENCE = OUT / "evidence_aventura_5735_current_strict.json"
PROPERTY_ID = 5735
PROPERTY_NAME = "Aventura Apartments"
WEBSITE = "https://aventuraorlandoapartments.prospectportal.com/"
CANONICAL_ADDRESS = "5300 Cinderlane Pkwy"
LISTING_URL = (
    "https://aventuraorlandoapartments.prospectportal.com/orlando/"
    "aventura-orlando-apartments/conventional/"
)
SOURCE_URL = (
    "https://aventuraorlandoapartments.prospectportal.com/orlando/"
    "aventura-orlando-apartments/floorplans/2-bed-2-bath-816984/"
...
def main() -> None:
    ...
    assert urlsplit(final_url).hostname == (
        "aventuraorlandoapartments.prospectportal.com"
    )
    ...
    listing_availability = [
        ...
        for item in (listing.get("clickable_elements") or [])
        if isinstance(item, dict)
        and "property_floorplan[id]=816984" in str((item.get("data") or {}).get("data-url") or "")
        and "property[id]=1025951" in str((item.get("data") or {}).get("data-url") or "")
    ]
    assert listing_availability

    text = normalized(html)
    assert "aventura orlando apartments" in text
    assert normalized(CANONICAL_ADDRESS) in text
    assert "orlando fl 32808" in text
    ...
    assert all(row.get("floor_plan_name") == "2 Bed 2 Bath" for row in native_rows)
    ...
    result = {
        ...
        "identity_evidence": {
            ...
            "current_listing_property_floorplan_binding": "816984",
            "current_listing_provider_property_binding": "1025951",
