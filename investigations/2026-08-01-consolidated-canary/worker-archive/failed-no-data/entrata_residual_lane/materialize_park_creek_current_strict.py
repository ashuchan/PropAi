#!/usr/bin/env python3
"""Materialize strict current evidence for Park Creek Manor."""
...
CAPTURE = OUT / "hb_modal_direct_9297_current.json"
EVIDENCE = OUT / "evidence_park_creek_9297_current_strict.json"
PROPERTY_ID = 9297
PROPERTY_NAME = "Park Creek Manor"
WEBSITE = "https://www.parkcreekmanor.com/"
CANONICAL_ADDRESS = "2520 Coombs Creek Dr"
LISTING_URL = (
...
def main() -> None:
    ...
    assert urlsplit(LISTING_URL).hostname == "www.parkcreekmanor.com"

    button = capture.get("published_button") or {}
    endpoint = str(capture.get("published_endpoint") or "")
    assert str(button.get("text") or "") == "2 Available"
    assert "property[id]=515100" in endpoint
    assert "property_floorplan[id]=897486" in endpoint
    ...
    assert "dallas tx 75211" in text
    ...
    assert {str(row["unit_number"]) for row in native_rows} == {"461", "244"}
    assert len(native_rows) == 2
    assert all(row.get("floor_plan_name") == "B-2" for row in native_rows)
    ...
    result = {
        ...
        "contamination_verdict": (
            ...
            "provider_count_native_ids_positive_rents"
        ),
        "identity_evidence": {
            ...
            "current_listing_provider_property_binding": "515100",
            "current_listing_property_floorplan_binding": "897486",
            "current_listing_published_unit_count": 2,
