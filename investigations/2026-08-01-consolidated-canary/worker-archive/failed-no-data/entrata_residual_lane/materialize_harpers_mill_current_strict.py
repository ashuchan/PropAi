#!/usr/bin/env python3
"""Materialize strict current evidence for The Pointe at Harpers Mill."""
...
CAPTURE = OUT / "hb_modal_direct_234772_current.json"
EVIDENCE = OUT / "evidence_harpers_mill_234772_current_strict.json"
PROPERTY_ID = 234772
PROPERTY_NAME = "The Pointe at Harpers Mill"
WEBSITE = "http://www.liveattp.com/"
CANONICAL_ADDRESS = "600 Harpers Mill Rd"
LISTING_URL = (
...
def main() -> None:
    ...
    assert urlsplit(LISTING_URL).hostname == "www.liveattp.com"

    button = capture.get("published_button") or {}
    endpoint = str(capture.get("published_endpoint") or "")
    assert str(button.get("text") or "").startswith("Available ")
    assert "property[id]=589277" in endpoint
    assert "property_floorplan[id]=633553" in endpoint
    ...
    assert "21108" in set(text.split())
    ...
    assert [str(row["unit_number"]) for row in native_rows] == ["511"]
    assert native_rows[0].get("floor_plan_name") == "Solo"
    ...
    result = {
        ...
        "identity_evidence": {
            ...
            "current_listing_provider_property_binding": "589277",
            "current_listing_property_floorplan_binding": "633553",
