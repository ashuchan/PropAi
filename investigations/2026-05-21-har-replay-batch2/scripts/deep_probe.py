        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return raw[:2_000_000].decode("latin-1", errors="ignore")
        except Exception:
            return ""
    return body
...
def probe_har(har_path: Path) -> dict[str, Any]:
    ...
    for ent in entries:
        ...
        if not decoded:
            continue
        # Truncate very long bodies for scoring (regex over 5 MB is slow)
        scored_body = decoded[:2_000_000]
        score_info = _score_body(scored_body, url)
