...
@pytest.mark.asyncio
async def test_cancelled_session_skips_wedged_local_teardown(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A property timeout must reach its finalizer even if CDP cannot close.

    ``asyncio.wait_for`` waits for a cancelled child to acknowledge
    cancellation, so wrapping a Playwright close is not a hard bound when the
    transport suppresses ``CancelledError``.  On an already-cancelling
    property task we must bypass local teardown and stop the paid remote
    session directly.
    """
    local_calls: list[str] = []
    remote_calls: list[str] = []

    class _CancellationResistantBrowser:
        async def close(self) -> None:
            local_calls.append("browser")
            while True:
                try:
                    await asyncio.Event().wait()
    ...
    class _CancellationResistantPlaywright:
        async def stop(self) -> None:
            local_calls.append("playwright")
            while True:
                try:
                    await asyncio.Event().wait()
                except asyncio.CancelledError:
    ...
    class _FakeHttpClient:
        ...
        async def put(self, url: str, **kwargs) -> None:
    ...
    session = hb._HbSession(mode="render")
    session.session_id = "sid-cancelled"
    session._browser = _CancellationResistantBrowser()
    session._pw = _CancellationResistantPlaywright()

    started = asyncio.Event()

    async def _property_work() -> None:
        try:
            started.set()
            await asyncio.Event().wait()
        finally:
    ...
    task = asyncio.create_task(_property_work())
    await started.wait()
    task.cancel()
    ...
    assert local_calls == []
    assert remote_calls == [
        "https://api.hyperbrowser.ai/api/session/sid-cancelled/stop"
    ]
