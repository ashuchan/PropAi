...
async def run_daily(
    ...
    log.info(
        f"Identity resolution: {sum(1 for i in identities if i.canonical_id)} resolved, "
    ...
    if profile_store is not None:
        for si, (_idx, _row, ident, _url) in enumerate(scrapeable):
            cid = ident.canonical_id
            if cid:
                try:
    ...
    if disappeared:
        _write_issues_jsonl(
            issues_path,
            [
                V.warning(V.PROPERTY_DISAPPEARED, f"property {c} missing from today's CSV", canonical_id=c)
    ...
    report = {
        ...
        "totals": {
            ...
            "scrape_successes": len(processed)
            - sum(1 for f in failed_properties if "SCRAPE_FAILED" in f.get("reason", "")),
        },
        "identity": {
            ...
            "by_source": {
                ...
                for src in ("unique_id", "property_id", "address_fp", "geo_fp", "website_fp", "unresolved")
            },
        },
...
def main():
    ...
    p.add_argument("--run-date", default=None, help="Override run date (YYYY-MM-DD); defaults to today")
