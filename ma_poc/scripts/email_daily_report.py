...
Reads the daily report straight out of Cloud SQL (the same instance the
jugnu runner writes to), renders an HTML body, and sends it via the
...
Required env (already in ma_poc/.env):
    ...
    CLOUD_SQL_INSTANCE      project:region:instance — required, no fallback
    DATABASE_URL            postgresql://user:pass@/dbname (host ignored)
...
Optional env:
    ...
    CLOUD_SQL_USE_IAM_TOKEN Set to ``1`` to authenticate to Postgres as
                            ...
                            password in DATABASE_URL. Requires the
                            principal to be a Cloud SQL IAM database
                            user with appropriate GRANTs.

Authentication — Application Default Credentials only. No IP whitelisting.

Both connection paths terminate inside Google's Cloud SQL frontend over
a TLS tunnel, so the instance's Authorized Networks list is irrelevant:

  1. Cloud SQL Connector (preferred, Linux/x64 only). Reads ADC from
     ``~/.config/gcloud/application_default_credentials.json`` and
     opens the tunnel in-process via aiohttp. Same path the production
     worker SA uses.
  2. cloud-sql-proxy fallback (Windows ARM64, where the connector's
     aiohttp dep has no prebuilt wheel). Spawns the Go binary as a
     child process; the proxy uses ADC and listens on a random
     localhost port that pg8000 connects to. Identical security model
     to path 1, just out-of-process.

Set ADC up once with:

    gcloud auth application-default login

…and confirm cloud-sql-proxy is on PATH (Windows ARM64 only):

    cloud-sql-proxy --version

Cloud SQL is the ONLY data source. There is no SQLite / local-file
fallback — running with ``CLOUD_SQL_INSTANCE`` unset is a hard error.
...
# root. Mirrors what the other scripts in this directory do.
...
def _make_engine_for_script() -> Any:
    """Open a SQLAlchemy engine bound to the Cloud SQL instance in .env.

    Hard requirement: ``CLOUD_SQL_INSTANCE`` must be set. Local fallbacks
    (SQLite, ad-hoc DATABASE_URLs) are explicitly disabled — the script's
    job is to email what the cloud runner produced, so accidentally
    reading a stale local DB would silently mis-report.

    Connection priority:
      1. ``google-cloud-sql-python-connector`` (preferred — handles auth
         + TLS + IAM transparently). Requires ``aiohttp`` to import.
      2. Direct ``pg8000`` connection to the instance's public IP, used
         on Windows ARM64 where the connector's ``aiohttp`` dep has no
         prebuilt wheel. The IP is read from ``CLOUD_SQL_PUBLIC_IP`` or
         resolved at runtime via ``gcloud sql instances describe``.
    """
    instance = os.getenv("CLOUD_SQL_INSTANCE", "").strip()
    if not instance:
        raise RuntimeError(
            "CLOUD_SQL_INSTANCE is not set in ma_poc/.env. The email "
            "script reads from Cloud SQL only — no local fallbacks."
        )
    ...
    # Same auth model as the connector: ADC + TLS tunnel, no Authorized
    # Networks / IP whitelisting required.
...
def main(argv: list[str] | None = None) -> int:
    ...
    engine = _make_engine_for_script()
    provider = SqlDataProvider(
        engine=engine,
        create_schema=False,  # Cloud SQL schema is alembic-managed.
