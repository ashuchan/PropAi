...
from ma_poc.fetch.stealth import IdentityPool  # noqa: E402
...
def extract_unit_link_candidates(html: str, base_url: str, max_n: int) -> list[str]:
...
def parse_strategies(spec: str) -> list[Strategy]:
    ...
    for tok in spec.split(","):
        ...
        if not m:
            raise ValueError(f"Bad strategy token: {tok!r}. Expected e.g. domcontentloaded:15000+5s")
        out.append(
            Strategy(
...
def load_urls_from_run(run_dir: Path, filter_expr: str | None, max_urls: int | None) -> list[tuple[str, str]]:
...
def write_outputs(
    ...
    for (pid, _page_key, strat), cap in cells.items():
            ...
            async with sem:
                for strategy in strategies:
                    ...
                    links = extract_unit_link_candidates(
                        home_html,
                        home_cap.final_url or url,
                        follow_links,
                    )
                    log.info("probe: %s [%s] discovered %d candidate link(s)", pid, strategy.slug, len(links))
                    for idx, link_url in enumerate(links, start=1):
                        pk = _link_page_key(link_url, idx)
                        log.info("probe: %s [%s] %s %s", pid, strategy.slug, pk, link_url)
...
def main() -> int:
    ...
    ap.add_argument(
        "--filter",
        help="k=v[,k=v...] applied to properties.json rows. Keys: tier, verdict, carry_forward_reason, scrape_outcome, or dotted path.",
    )
    ...
    ap.add_argument("--api-catalogue", type=Path, default=_MA_POC_ROOT / "config" / "api_catalogue.json")
    ...
    log.info("probing %d url(s) × %d strategy(ies) -> %s", len(urls), len(strategies), out_dir)
