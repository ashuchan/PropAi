...
# PR-2/PR-3: Phase 1+2 classes + PhaseContext + PhaseRegistry (no circular dep)
...
# Phase 3-7 import entrata functions, so they are imported lazily inside scrape()
# to avoid the circular import: entrata → phase3 → entrata.
...
async def scrape(
    ...
    async with BrowserSession(proxy=proxy) as _session:
        ...
        try:
            ...
            # ════════════════════════════════════════════════════════════
            # PHASES 3-7: Class-based extraction pipeline
            # ════════════════════════════════════════════════════════════
            # Deferred imports break the circular dep (phase3/6/7 → entrata).
            from extraction.engine.phases.phase3_known_patterns import Phase3KnownPatternExtraction  # noqa: E402
            from extraction.engine.phases.phase4_link_exploration import Phase4LinkExploration  # noqa: E402
            from extraction.engine.phases.phase5_llm_api import Phase5LlmApiAnalysis  # noqa: E402
            from extraction.engine.phases.phase6_dom_fallback import Phase6DomFallback  # noqa: E402
            from extraction.engine.phases.phase7_finalize import Phase7Finalize  # noqa: E402

            _ctx.skip_to_tier = skip_to_tier
            _ctx.run_full_cascade = run_full_cascade
            _ctx.property_id = results.get("_property_id", "unknown")

            await Phase3KnownPatternExtraction().run(_ctx)
            await Phase4LinkExploration().run(_ctx)
            await Phase5LlmApiAnalysis().run(_ctx)
            await Phase6DomFallback().run(_ctx)
            await Phase7Finalize().run(_ctx)

            # Propagate ctx outputs back to results dict.
            results["extraction_tier_used"] = _ctx.extraction_tier_used
            results["_winning_page_url"] = _ctx.winning_page_url
            results["property_links_crawled"] = _ctx.property_links_crawled
            results["api_calls_intercepted"] = [r["url"] for r in api_responses]
            results["_llm_analysis_results"] = _ctx.llm_analysis_results
            results["_explored_links"] = _ctx.explored_links_status
