    ...
    *,
    ...
    if not result.get("concessions_text"):
        try:
            ...
            # ``ctx`` is created after detection, but concession capture runs
            # earlier so it can enrich every extraction path.  Use the
            # explicit captures (or fetcher network log) directly here.
            _captured = api_responses or getattr(fetch_result, "network_log", []) or []
