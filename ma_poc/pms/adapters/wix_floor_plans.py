...
class WixFloorPlansAdapter:
    ...
    async def extract(self, page: Page, ctx: AdapterContext) -> AdapterResult:
        ...
        if not callable(evaluate):
            ...
            result.errors.append("wix_floor_plans: no live page")
            return result
        try:
        ...
        if not isinstance(payload, dict) or not payload.get("ok"):
            result.confidence = 0.0
            result.errors.append("wix_floor_plans: no 'Starting at $X' plan cards found")
            return result
        cards = payload.get("cards") or []
        if not isinstance(cards, list) or not cards:
            result.confidence = 0.0
            result.errors.append("wix_floor_plans: zero cards in payload")
            return result
