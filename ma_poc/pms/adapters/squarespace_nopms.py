...
class SquarespaceNoPmsAdapter:
    ...
    async def extract(self, page: Page, ctx: AdapterContext) -> AdapterResult:
        ...
        if units:
            ...
            if pp.n_admitted > 0:
                # Use main's strict-partition convention (list(pp.units) +
                # pp.to_meta()) to match other adapters and the D16
                # invariant — feature branch's `pp.admitted` returns the
                # combined unit+plan list, which would conflate the two
                # partitions in downstream consumers.
