...
async def _t1(s: BrowserSession, catalogue: Optional[dict[str, Any]]) -> ExtractionResult:
    return await tier1_api.extract(s, catalogue)


async def _t2(s: BrowserSession, catalogue: Optional[dict[str, Any]]) -> ExtractionResult:
    return await tier2_jsonld.extract(s)


async def _t3(s: BrowserSession, catalogue: Optional[dict[str, Any]]) -> ExtractionResult:
    return await tier3_templates.extract(s)


async def _t4(s: BrowserSession, catalogue: Optional[dict[str, Any]]) -> ExtractionResult:
